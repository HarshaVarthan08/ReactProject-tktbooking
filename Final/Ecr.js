...
const Ecr = () => {
  const roomsData = [
    { id: 2, name: 'Standard Room', cinename:'MURURGAN CINEMAS',price: 200, location: 'ECR,MAIN ROAD', image:joe,movie:'JOE',showdate:'23/12/23',showtime:'5:00AM'},
    
    { id: 1, name: 'Executive Suite',cinename:'MAYAJAL',price: 250, location: 'ECR,ROYAL STREET', image:salar,movie:'SALAR',showdate:'23/12/23',showtime:'5:00AM'},
  ];
  ...
  const handleRoomCountChange = (value, roomType) => {
    if (roomType === 'executive') {
      setExecutiveRoomCount(Math.max(0, executiveRoomCount + value));
    } else if (roomType === 'deluxe') {
      setDeluxeRoomCount(Math.max(0, deluxeRoomCount + value));
    }
  };
              ...
              <li key={room.id} className="room-card">
                ...
                <h3>{room.cinename}</h3>
                  ...
                  <div style={{margin:'1px'}}>
                    ...
                    <p>Number of {room.moviename} Tickets: {room.name === 'Executive Suite' ? executiveRoomCount : deluxeRoomCount}</p>
                    <br></br>
